...
module FB3Bookmarks {
	...
	export class LitResBookmarksProcessor implements IBookmarks {
		...
		constructor(public FB3DOM: FB3DOM.IFB3DOM, LitresSID?: string, LitresLocalXML?: string, UseCatalit2: boolean = false) {
			if (LitresSID) {
				this.SID = LitresSID;
			}
			this.xhrIE9 = false;
		...
		private OnGetNotesSuccess(Result: ICatalitResponse): void {
			...
			var NewBookmark = null, Note = null;

			if (Notes) {
                for (var j = 0; j < Notes.length; j++) {
                    Note = Notes[j];
                    NewBookmark = new Bookmark(this);
		...
		public ReLoad(SaveAutoState?: boolean) {
			var TemporaryNotes = new LitResBookmarksProcessor(this.FB3DOM, this.SID, undefined, this.UseCatalit2);
	...
	export class Bookmark implements IBookmark {
		...
		private MakeXPathSub(str: string) {
