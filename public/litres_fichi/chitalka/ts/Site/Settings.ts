...
function setTheme() {
	...
	if (getSetting('theme') < 4) setUINightLogo(true);
	else setUINightLogo(false);	
}
...
function setUINightLogo(setState) {
	if (setState) addClass(doc.body, 'night_logo');
	else removeClass(doc.body, 'night_logo');
}
...
function changeFontFamily() {
	...
	changeCSS('#zoomedImg .readerStyles', 'font-family', fontList[fontVal][0],
		fontList[fontVal][1] ? ', ' + fontList[fontVal][1] : '');
	setCiteFontSize();
...
function changeReaderMargin() {
	...
	if (!LitresURLParser.Modal) {
		...
		if(readerHeight > 0 && pda.state && !aldebaran_or4) {
			...
			changeCSS('#footer', 'margin-top', (win.innerHeight - readerHeight - 30 - trialsGap - footerTopPda) + 'px;');
		}
...
function setCustomFontColor(clicked?: boolean) {
...
function setCustomBackgroundColor(clicked?: boolean) {
	setSetting(1, 'customColors');
...
function calcSettingsArrowPos() {
	var settingsWinRect = (<HTMLElement> doc.querySelector('#settings')).getBoundingClientRect();
	var settingsButtonRect = (<HTMLElement> doc.querySelector('.menu-settings')).getBoundingClientRect();
